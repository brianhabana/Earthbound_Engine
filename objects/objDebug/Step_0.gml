...
if active {
    if can_input {
        ...
        if keyboard_check_pressed(global.kb_enter) {
            var command = string_lower(keyboard_string)
            var args = ds_list_create()
            //Split string
            var _pos = string_pos(" ", command)
            if _pos == 0 {
                ds_list_add(args, command)
            } else {
                while _pos > 0 {
                    var _arg = string_delete(command, _pos , string_length(command))
                    command = string_delete(command, 1, _pos)
                    _pos = string_pos(" ", command)
                    ds_list_add(args, _arg)
                    if _pos == 0 and command != "" {
                        ds_list_add(args, command)
                    }
                }
            }
